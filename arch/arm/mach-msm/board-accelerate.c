...
/* GPIO_INDEX			Rev	{#00, #01, #02, #03, #04, #05, #06 }, */
	/* MDP_VSYNC	*/	{  0,  0,  0,  0,  0,  0 },
	/* VOLUME_UP	*/	{ 50, 50, 50, 50, 50, 50 },
	/* VOLUME_DOWN	*/	{ 81, 81, 81, 81, 81, 81 },
	/* GPIO_MAG_RST */	{ 48, 48, 48, 48, 48, 48 },
	/* LCD_22V_EN */	{  4,  4,  4,  4,  4,  4 },
	/* LINEOUT_EN */	{  5,  5,  5,  5,  5,  5 },
	/* A2220_WAKEUP */	{ 35, 35, 35, 35, 35, 35 },
	/* A2220_SDA */		{ 36, 36, 36, 36, 36, 36 },
	/* A2220_SCL */		{ 37, 37, 37, 37, 37, 37 },
	/* CAM_AF_EN */		{ 77, 77, 77, 77, 77, 77 },
	/* CAM_FLASH_SW */	{ 49, 49, 49, 49, 49, 49 },
	/* BT_HOST_WAKE */	{ 10, 10, 10, 10, 10, 10 },
	/* BT_WAKE */		{ 79, 79, 79, 79, 79, 79 },
	/* BT_EN */		{ 82, 82, 82, 82, 82, 82 },
};
...
int gpio_rev(unsigned int index)
{
	...
	if (system_rev < BOARD_REV06)
		...
		return gpio_table[index][BOARD_REV06];
}
...
#define MSM_PMEM_ADSP_SIZE         0x8000000 /* 128 Mbytes */
...
static void opt_init(void)
{
	...
	gpio_tlmm_config(GPIO_CFG(GPIO_SENSOR_ALS_SCL, 0, GPIO_CFG_INPUT,
		GPIO_CFG_NO_PULL, GPIO_CFG_2MA), 1);
	ret = gpio_request(GPIO_ALS_INT, "PSVOUT");
	if (ret) {
		pr_err("%s gpio request %d err\n", __func__, GPIO_ALS_INT);
	} else {
...
{
	int ret = 0;
	gpio_tlmm_config(GPIO_CFG(gpio_rev(GPIO_MAG_RST), 0,
		GPIO_CFG_OUTPUT, GPIO_CFG_PULL_UP, GPIO_CFG_2MA), 1);
	sensor_power_on_vdd(SNS_PWR_ON, SNS_PWR_ON);
	ret = gpio_request(gpio_rev(GPIO_MAG_RST), "MAG_RST");
	if (ret) {
		pr_err("%s gpio request %d err\n", __func__,
			gpio_rev(GPIO_MAG_RST));
	} else {
		gpio_direction_output(gpio_rev(GPIO_MAG_RST), 0);
		usleep_range(20, 20);
		gpio_set_value(gpio_rev(GPIO_MAG_RST), 1);
	}
...
static struct platform_device *accelerate_devices[] __initdata = {
	&msm_8960_q6_lpass,
	&msm_8960_q6_mss_fw,
	&msm_8960_q6_mss_sw,
	&msm_8960_riva,
...
static struct max8952_platform_data accelerate_max8952_pdata = {
...
static struct i2c_board_info cmc624_max8952_i2c_borad_info[] = {
...

	{
		...
		.platform_data = &accelerate_max8952_pdata,
	},
};
#endif /*CONFIG_REGULATOR_MAX8952*/
...
static void __init samsung_accelerate_init(void)
{
#ifdef CONFIG_SEC_DEBUG
	sec_debug_init();
#endif
	...
	platform_add_devices(accelerate_devices, ARRAY_SIZE(accelerate_devices));
...
MACHINE_START(ACCELERATE, "SAMSUNG ACCELERATE")
	.map_io = msm8960_map_io,
	.reserve = msm8960_reserve,
	.init_irq = msm8960_init_irq,
	.handle_irq = gic_handle_irq,
	.timer = &msm_timer,
	.init_machine = samsung_accelerate_init,
