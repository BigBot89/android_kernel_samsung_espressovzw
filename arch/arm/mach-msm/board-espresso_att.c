...
static void irda_device_init(void)
{
	...
	if (!reg_l29) {
		...
		if (IS_ERR(reg_l29)) {
			printk(KERN_ERR "%s: could not get 8921_l29, rc = %ld\n",
				__func__, PTR_ERR(reg_l29));
			return;
		}
		ret = regulator_set_voltage(reg_l29, 2000000, 2000000);
		if (ret) {
			pr_err("%s: unable to set ldo29 volagate 2.0V\n",
				__func__);
			return;
		}
	}
	...
	if (ret) {
		printk(KERN_ERR "%s: enable l29 failed, rc=%d\n",
				__func__, ret);
		return;
	}
	printk(KERN_ERR "%s irda_regualater 2V on is finished.\n",
				__func__);
...
static struct platform_device *espresso_att_devices[] __initdata = {
	&msm_8960_q6_lpass,
	&msm_8960_q6_mss_fw,
	&msm_8960_q6_mss_sw,
	&msm_8960_riva,
...
static void __init register_i2c_devices(void)
{
	...
	if (machine_is_msm8960_cdp())
		...
		mach_mask = I2C_FFA;
	else if (machine_is_ESPRESSO_ATT())
		mach_mask = I2C_FFA;
	else
...
static void __init samsung_espresso_att_init(void)
{
#ifdef CONFIG_SEC_DEBUG
	sec_debug_init();
#endif
...
#endif
	usb_switch_init();
	platform_add_devices(espresso_att_devices,
			ARRAY_SIZE(espresso_att_devices));
...
MACHINE_START(ESPRESSO_ATT, "SAMSUNG ESPRESSO")
	.map_io = msm8960_map_io,
	.reserve = msm8960_reserve,
	.init_irq = msm8960_init_irq,
	.handle_irq = gic_handle_irq,
	.timer = &msm_timer,
	.init_machine = samsung_espresso_att_init,
