...
unsigned int gpio_table[][GPIO_REV_MAX] = {
	...
	/* ALS_SDA */		{ 63, 63, 12 },
	/* ALS_SCL */		{ 64, 64, 13 },
	/* A2220_I2C_SDA */	{ 12, 12, 36 },
	/* A2220_I2C_SCL */	{ 13, 13, 37 },
#ifdef CONFIG_KEYBOARD_CYPRESS_TOUCH_236
	/* GPIO_TOUCH_KEY_EN */	{ 94, 94, 51 },
	/* GPIO_TOUCH_KEY_INT*/ { 54, 54, 52 },
#endif
	/*CAM_KEY*/		{ 98, 98, 43 },
};
...
{
	if (onoff)
		gpio_direction_output(gpio_rev(GPIO_TOUCH_KEY_EN), 1);
	else
		gpio_direction_output(gpio_rev(GPIO_TOUCH_KEY_EN), 0);
}

static u8 touchkey_keycode_common[] = {KEY_BACK, KEY_HOME, KEY_MENU};
...
static int __init sensor_device_init(void)
{
	...
	defined(CONFIG_MPU_SENSORS_MPU6050B1) || \
	defined(CONFIG_INPUT_BMP180) || defined(CONFIG_OPTICAL_GP2A) || \
	defined(CONFIG_OPTICAL_TAOS_TRITON) || defined(CONFIG_INPUT_YAS_SENSORS)
...
static struct platform_device *gogh_devices[] __initdata = {
	&msm_8960_q6_lpass,
	&msm_8960_q6_mss_fw,
	&msm_8960_q6_mss_sw,
	&msm_8960_riva,
...
{	/*KEY REV*/
	...
	gpio_keys_button[2].gpio = gpio_rev(CAM_KEY);
...
#ifdef CONFIG_KEYBOARD_CYPRESS_TOUCH_236
	cypress_touchkey_pdata.gpio_int = gpio_rev(GPIO_TOUCH_KEY_INT);
	if (system_rev >= BOARD_REV02)
		cypress_touchkey_pdata.touchkey_keycode =
					touchkey_keycode_rev02;
	touchkey_i2c_devices_info[0].irq =
		MSM_GPIO_TO_INT(gpio_rev(GPIO_TOUCH_KEY_INT));
#endif
#ifdef CONFIG_YAS_ACC_MULTI_SUPPORT
	if (system_rev < BOARD_REV03) {
		accel_pdata.used_chip = K3DH_ENABLED;
		accel_pdata.position = 1;
		sns_i2c_borad_info[0].addr = 0x19;
	}
#endif
}

static void __init samsung_gogh_init(void)
{
#ifdef CONFIG_SEC_DEBUG
	sec_debug_init();
#endif
	...
	platform_add_devices(gogh_devices, ARRAY_SIZE(gogh_devices));
	msm8960_init_hsic();
	msm8960_init_cam();
	msm8960_init_mmc();
...
MACHINE_START(GOGH, "SAMSUNG GOGH")
	.map_io = msm8960_map_io,
	.reserve = msm8960_reserve,
	.init_irq = msm8960_init_irq,
	.handle_irq = gic_handle_irq,
	.timer = &msm_timer,
	.init_machine = samsung_gogh_init,
