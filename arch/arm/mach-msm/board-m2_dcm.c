...
unsigned int gpio_table[][GPIO_REV_MAX] = {
	...
	/* LCD_22V_EN */		{ 10,  4,  4},
	/* CAM_AF_EN */			{ 54,  0, 77},
	/* BT_HOST_WAKE */		{ -1, 10, 10},
	/* BT_WAKE */			{ -1, 79, 79},
	/* BT_EN */			{ -1, 52, 82},
	/* CAM2_RST_N */		{ 26, 55, 76},
	/* GPIO_MAIN_MIC_BIAS */	{ 18, 32, 75},
	/* GPIO_MHL_EN */		{ 19, 33, 33},
	/* CAM_CORE_EN */		{  6,  6, 32},
	/* CAM_FLASH_SW */		{ 49, 49, 80},
	/* ALS_INT */			{ 42, 42,  6},
};
...
#define MSM_PMEM_ADSP_SIZE                 0x8800000 /* 136 Mbytes */
#define MSM_PMEM_ADSP_SIZE_FOR_2GB         0x8800000 /* 136 Mbytes */
...
static void msm8960_mhl_gpio_init(void)
{
	...
	ret = gpio_request(gpio_rev(GPIO_MHL_EN), "mhl_en");
	if (ret < 0) {
	...
	gpio_set_value_cansleep(gpio_rev(GPIO_MHL_EN), 0);
	gpio_set_value_cansleep(GPIO_MHL_RST, 0);

	gpio_tlmm_config(GPIO_CFG(gpio_rev(GPIO_MHL_EN), 0, GPIO_CFG_OUTPUT,
...
static void sii9234_hw_onoff(bool onoff)
{
	...
	if (onoff) {
		...
		if (gpio_direction_output(gpio_rev(GPIO_MHL_EN), 1))
			pr_err("%s error in making GPIO_MHL_EN High\n"
			, __func__);
	} else {
		...
		if (gpio_direction_output(gpio_rev(GPIO_MHL_EN), 0))
			pr_err("%s error in making GPIO_MHL_EN Low\n"
			, __func__);
		if (mhl_l12) {
...
static void isdbt_gpio_init(void)
{
	...
	ret = gpio_request(PM8921_GPIO_PM_TO_SYS(PMIC_GPIO_ISDBT_EN), \
							"ISDBT_PWR_EN");
	if (ret) {
		pr_err("%s: Failed to request gpio %d\n", __func__,
			PM8921_GPIO_PM_TO_SYS(PMIC_GPIO_ISDBT_EN));
		return ret;
	}

	ret = pm8xxx_gpio_config(PM8921_GPIO_PM_TO_SYS(PMIC_GPIO_ISDBT_EN), \
								&param);
	if (ret) {
	...
	gpio_set_value_cansleep(PM8921_GPIO_PM_TO_SYS(PMIC_GPIO_ISDBT_EN), 0);

	ret = gpio_request(PM8921_GPIO_PM_TO_SYS(PMIC_GPIO_ISDBT_RST), \
							"ISDBT_RST");
	if (ret) {
		pr_err("%s: Failed to request gpio %d\n", __func__,
			PM8921_GPIO_PM_TO_SYS(PMIC_GPIO_ISDBT_RST));
		return ret;
	}

	ret = pm8xxx_gpio_config(PM8921_GPIO_PM_TO_SYS(PMIC_GPIO_ISDBT_RST), \
							&param);
	if (ret) {
		pr_err("%s: Failed to configure gpio %d\n", __func__,
			PM8921_GPIO_PM_TO_SYS(PMIC_GPIO_ISDBT_RST));
	...
	gpio_tlmm_config(GPIO_CFG(GPIO_ISDBT_INT, GPIOMUX_FUNC_GPIO,
		GPIO_CFG_INPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA),
		GPIO_CFG_ENABLE);
	gpio_tlmm_config(GPIO_CFG(GPIO_ISDBT_SPI_MOSI, GPIOMUX_FUNC_GPIO,
		GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_8MA),
		GPIO_CFG_ENABLE);
	gpio_tlmm_config(GPIO_CFG(GPIO_ISDBT_SPI_MISO, GPIOMUX_FUNC_GPIO,
		GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_8MA),
		GPIO_CFG_ENABLE);
	gpio_tlmm_config(GPIO_CFG(GPIO_ISDBT_SPI_CS, GPIOMUX_FUNC_GPIO,
		GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_8MA),
		GPIO_CFG_ENABLE);
...
static void isdbt_gpio_on(void)
{
	...
	gpio_tlmm_config(GPIO_CFG(GPIO_ISDBT_INT, GPIOMUX_FUNC_GPIO,
		...
		GPIO_CFG_ENABLE);
	gpio_tlmm_config(GPIO_CFG(GPIO_ISDBT_SPI_MOSI, GPIOMUX_FUNC_1,
		GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_8MA),
		GPIO_CFG_ENABLE);
	gpio_tlmm_config(GPIO_CFG(GPIO_ISDBT_SPI_MISO, GPIOMUX_FUNC_1,
		GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_8MA),
		GPIO_CFG_ENABLE);
	gpio_tlmm_config(GPIO_CFG(GPIO_ISDBT_SPI_CS, GPIOMUX_FUNC_1,
		GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_8MA),
		GPIO_CFG_ENABLE);
	gpio_tlmm_config(GPIO_CFG(GPIO_ISDBT_SPI_CLK, GPIOMUX_FUNC_1,
		GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_8MA),
		GPIO_CFG_ENABLE);
...
static void isdbt_gpio_off(void)
{
	...
	gpio_free(PM8921_GPIO_PM_TO_SYS(PMIC_GPIO_ISDBT_RST));
	gpio_free(PM8921_GPIO_PM_TO_SYS(PMIC_GPIO_ISDBT_EN));
}
...
static void hall_pmgoio_config(void)
{
	int err;
	...
	err = pm8xxx_gpio_config(PM8921_GPIO_PM_TO_SYS(PMIC_GPIO_HALL_SW_N),
			&param);
	if (err < 0)
		pr_err("umfa.hall fail to pm8xxx_gpio_config\n");

}

struct hall_sw_pdata {
	void (*pmic_gpio_config) (void);
	int wakeup;
	int gpio;
};

static struct hall_sw_pdata hall_sw_dev_data = {
	.wakeup			= 1,
	.gpio			= PM8921_GPIO_PM_TO_SYS(PMIC_GPIO_HALL_SW_N),
	.pmic_gpio_config = hall_pmgoio_config,
};

static struct platform_device hall_sw_device = {
	.name = "hall_sw",
	.id = PM8921_GPIO_PM_TO_SYS(PMIC_GPIO_HALL_SW_N),
	.dev	= {
		.platform_data = &hall_sw_dev_data,
	},
};
#endif
...
static struct pm_gpio wcd9310_power_en = {
	.direction	= PM_GPIO_DIR_OUT,
	.output_buffer	= PM_GPIO_OUT_BUF_CMOS,
	.output_value	= 1,
	.pull		= PM_GPIO_PULL_NO,
...
static void codec_power_en_gpio_init()
{
	...
	ret = gpio_request(PM8921_GPIO_PM_TO_SYS(PMIC_GPIO_LINEOUT_EN),
			"pmic_gpio_lineout_en");
	if (ret < 0) {
		pr_err("pmic_gpio_lineout_en gpio_request is failed\n");
		return;
	}

	ret = pm8xxx_gpio_config(
		PM8921_GPIO_PM_TO_SYS(PMIC_GPIO_LINEOUT_EN),
		&wcd9310_power_en);
	if (ret) {
		pr_err("%s : pmic_gpio_lineout_en config failed\n", __func__);
		return;
	}
}
...
static void __init samsung_m2_dcm_init(void)
{
	...
	if (system_rev < BOARD_REV03) {
		pr_info("%s : system rev = %d, CODEC power LINEOUT EN\n",
			__func__, system_rev);
			codec_power_en_gpio_init();
	}
