...
struct acc_con_info {
	...
	int dock_irq;
	int mhl_irq;
	struct device *dock_intr;
};

int dock_state;

static ssize_t dock_intr_show(struct device *dev,
...
static DEVICE_ATTR(dock_intr, 0664, dock_intr_show, NULL);
...
static int acc_con_probe(struct platform_device *pdev)
{
	...
	dock_state = acc->current_dock;
	acc->dock_intr = device_create(sec_class, NULL, 0, acc, "dock_intr");
	if (acc->dock_intr < 0) {
		printk(KERN_ERR "%s: Failed to create device (dock_intr)!\n",
				__func__);
		goto err_sw_dock;
	}

	ret = device_create_file(acc->dock_intr, &dev_attr_dock_intr);
	if (ret < 0) {
		printk(KERN_ERR "failed to create device file(%s)!\n",
				dev_attr_dock_intr.attr.name);
		goto err_sw_dock;
	}
