...
void set_reactive_alert(int onoff)
{
	struct mpu_private_data *mpu =
	    (struct mpu_private_data *) i2c_get_clientdata(this_client);
	struct i2c_client *client = mpu->client;
...
void check_reactive_alert()
{
	...
	if (mldl_cfg->inv_mpu_state->accel_reactive) {
		unsigned char mpu_register;
		unsigned char mpu_strbuf[100];
		int adaptnum = pdata_slave[EXT_SLAVE_TYPE_ACCEL]->adapt_num;

		slave_adapter =
			i2c_get_adapter(adaptnum);
		inv_serial_read(slave_adapter, 0x68,
			MPUREG_PWR_MGMT_1, 1, &mpu_register);
		snprintf(mpu_strbuf, 100, "R%x:%x,", MPUREG_PWR_MGMT_1,
			mpu_register);
		inv_serial_read(slave_adapter, 0x68,
			MPUREG_PWR_MGMT_2, 1, &mpu_register);
		snprintf(mpu_strbuf, 100, "%s R%x:%x, ", mpu_strbuf,
			MPUREG_PWR_MGMT_2, mpu_register);
		inv_serial_read(slave_adapter, 0x68,
			MPUREG_CONFIG, 1, &mpu_register);
		snprintf(mpu_strbuf, 100, "%s R%x:%x, ", mpu_strbuf,
			MPUREG_CONFIG, mpu_register);
		inv_serial_read(slave_adapter, 0x68,
			MPUREG_ACCEL_CONFIG, 1, &mpu_register);
		snprintf(mpu_strbuf, 100, "%s R%x:%x, ", mpu_strbuf,
			MPUREG_ACCEL_CONFIG, mpu_register);
		inv_serial_read(slave_adapter, 0x68,
			MPUREG_INT_ENABLE, 1, &mpu_register);
		snprintf(mpu_strbuf, 100, "%s R%x:%x, ", mpu_strbuf,
			MPUREG_INT_ENABLE, mpu_register);
		inv_serial_read(slave_adapter, 0x68,
			MPUREG_INT_ENABLE, 1, &mpu_register);
		pr_err("MPU_REG %s R%x:%x\n", mpu_strbuf,
			MPUREG_INT_ENABLE, mpu_register);
