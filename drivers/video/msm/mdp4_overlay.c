...
static void mdp4_overlay_bg_solidfill_clear(struct mdp4_overlay_pipe *pipe)
{
...
static void mdp4_mixer_stage_commit(int mixer)
{
	...
	if (ctrl->mixer_cfg[mixer] != cfg[mixer]) {
		...
		if (ctrl->mixer_cfg[MDP4_MIXER2] != cfg[MDP4_MIXER2]) {
			...
			outpdw(MDP_BASE + off, data); /* LAYERMIXER_IN_CFG */
			msleep(20);
			/* it happens only when connecting
			disconnecting the dongle with the wfd */
		}
	} else {
		if (mixer == MDP4_MIXER0) {
			if ((ctrl->panel_mode & MDP4_PANEL_DSI_VIDEO) ||
	...
	struct msm_fb_data_type *mfd, uint32 perf_level)
{
	...
	if (cam_mode) {
		pr_info(" camcorder mode = 1 . do not use_blt.\n");
		use_blt = 0;
	}
#endif
	return use_blt;
...
int mdp4_overlay_unset(struct fb_info *info, int ndx)
{
	...
	if (mfd->mdp_rev >= MDP_REV_41 &&
		...
		if (!mdp4_overlay_is_rgb_type(pipe->src_format))
			mdp4_overlay_bg_solidfill_clear(pipe);
