...
#ifdef MDP_CLK_CONTROL
static int mdp_blk_overlay_on_cnt;
struct mutex mutex_overlay_out;
#endif
...
#ifdef CLK_ONOFF_PROTECTION
static int mipi_dsi_cmd_mode_on;
static int mipi_dsi_cmd_mode_off;
#endif

#ifdef CLK_ONOFF_PROTECTION
#define TOUT_PERIOD	(4*HZ)	/* 4 second */
#else
...
inline void printkout(offset, cnt)
{
	int i = 0;
...
void mdp4_dsi_cmd_dma_busy_wait(struct msm_fb_data_type *mfd)
{
...
#ifdef CLK_ONOFF_PROTECTION
	mutex_lock(&dsi_cmd_clk_on_off);
	mipi_dsi_cmd_mode_on = 1;
#endif

	if (dsi_clock_timer.function) {
#ifndef CLK_ONOFF_PROTECTION
	...
	mutex_unlock(&dsi_cmd_clk_on_off);
#endif
...
#ifdef MDP_CLK_CONTROL
	mutex_lock(&mutex_overlay_out);
	while (loop_cnt--) {
		if (mdp_blk_overlay_on_cnt) {
			pr_info("[cmd_lockup]overlay data out is not done yet: overlay %d, dmap %d\n",
			mdp_blk_overlay_on_cnt);
			usleep(5000);
		}
	}
...
#ifdef MDP_CLK_CONTROL
	mutex_unlock(&mutex_overlay_out);
#endif
